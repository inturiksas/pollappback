...
public class CountryDAO implements ICountryDAO {
	...
	public List<Country> findByProperty(String propertyName,
			final Object value, final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding Country instance with property: "
				+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select country from Country country where country."
	...
	public List<Country> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all Country instances", Level.INFO,
				null);
		try {
			final String queryString = "select country from Country country";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
